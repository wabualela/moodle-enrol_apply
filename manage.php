...
$userenrolment  = optional_param('userenrolment', null, PARAM_INT);
...
if ($formaction != null && $userenrolment != null) {

    $enrolapply = enrol_get_plugin('apply');
    switch ($formaction) {
        case 'confirm':
            $enrolapply->confirm_enrolment([$userenrolment]);
            \core\notification::success(get_string('enrolmentconfirmed', 'enrol_apply'));
            break;
        case 'wait':
            $enrolapply->wait_enrolment([$userenrolment]);
            \core\notification::success(get_string('enrolmentwaited', 'enrol_apply'));
            break;
        case 'cancel':
            $enrolapply->cancel_enrolment([$userenrolment]);
            \core\notification::success(get_string('enrolmentcanceled', 'enrol_apply'));
            break;
    }
    redirect($manageurl);
}
